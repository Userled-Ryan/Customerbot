...
| **Move to Dev Action** | Lane → Dev Action · records the dev on support as the ticket's dev owner and assigns them the Linear issue · DMs every `SUPPORT_HANDLE` member the handoff · appends OUTBOUND comms event | [`application/tracking/lane_handoff.py`](https://github.com/Userled-Ryan/Customerbot/blob/main/src/customerbot/application/tracking/lane_handoff.py) |
